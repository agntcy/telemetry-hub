...
class TestCacheMetric(BaseMetric):
    ...
    async def compute(self, session: SessionEntity, context=None):
        return MetricResult(
            metric_name=self.name, value=0.5, aggregation_level="session",
...
class TestAgentCacheFunctionality:
        ...
        cached_metrics = [
            ...
            },
            {
                "metrics": {
                    "metric_name": "test_cache_metric", 
                    "aggregation_level": "session",
                    "metadata": {"agent_id": "agent_1", "session_id": "test_session_123"}
                }
            }
        ]
        
        # Test - should only return the session metric without agent_id
        found, result = self.metric._check_session_cache(cached_metrics, "test_cache_metric")
        ...
        cached_metrics = [
            ...
            },
            {
                "metrics": {
                    ...
                    "edges_involved": [],
                    "success": True,
                    "metadata": {"agent_id": "agent_2", "session_id": "test_session_123"}  
                }
            }
        ]
        
        # Test - should return agent results for the metric
        results = self.metric._check_all_agents_cache(cached_metrics, "test_cache_metric")
    ...
    def test_check_agent_cache_specific_agent(self):
        ...
        result = self.metric._check_agent_cache(cached_metrics, "test_cache_metric", "agent_1")
    ...
    def test_check_agent_cache_no_match(self):
        ...
        result = self.metric._check_agent_cache(cached_metrics, "test_cache_metric", "agent_999")
        assert result is None
