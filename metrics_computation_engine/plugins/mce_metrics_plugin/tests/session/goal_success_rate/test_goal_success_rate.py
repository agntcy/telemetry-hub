...
def make_agent_span(entity_type, entity_name, span_id, agent_id, input_payload=None, output_payload=None):
...
async def test_goal_success_rate_agent_computation_single_agent():
    ...
    spans = [
        ...
        make_agent_span("llm", "gpt-4", "llm1", "knowledge_agent", llm_input, llm_output),
    ]
...
async def test_goal_success_rate_agent_computation_multiple_agents():
    ...
    spans = [
        make_agent_span("agent", "knowledge_agent", "agent1", "knowledge_agent"),
        make_agent_span("llm", "gpt-4", "llm1", "knowledge_agent", llm_input1, llm_output1),
        make_agent_span("agent", "translation_agent", "agent2", "translation_agent"),
        make_agent_span("llm", "gpt-4", "llm2", "translation_agent", llm_input2, llm_output2),
    ]
    ...
    knowledge_result = next((r for r in results if r.metadata["agent_id"] == "knowledge_agent"), None)
    translation_result = next((r for r in results if r.metadata["agent_id"] == "translation_agent"), None)

    assert knowledge_result is not None
    assert translation_result is not None

    assert knowledge_result.metadata["agent_input_query"] == "What is the capital of Spain?"
    assert knowledge_result.metadata["agent_final_response"] == "The capital of Spain is Madrid."
