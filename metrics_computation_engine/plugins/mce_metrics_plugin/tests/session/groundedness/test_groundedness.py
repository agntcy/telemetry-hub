...
class TestGroundedness:
    ...
    async def test_groundedness_session_level_success(self):
        ...
        metric = Groundedness()
        mock_jury = Mock()
        mock_jury.judge = Mock(return_value=(1, "Response is fully grounded in provided context"))
    ...
    async def test_groundedness_session_level_failure(self):
        ...
        metric = Groundedness()
        mock_jury = Mock()
        mock_jury.judge = Mock(return_value=(0, "Response contains unverifiable claims"))
        metric.jury = mock_jury
    ...
    async def test_groundedness_agent_computation(self):
        ...
        with patch.object(type(session), 'agent_stats', new_callable=lambda: property(lambda self: {"agent_a": {}, "agent_b": {}})), \
             patch('mce_metrics_plugin.session.groundedness.SessionEntity.get_agent_conversation_text', side_effect=mock_get_agent_conversation_text), \
             patch('mce_metrics_plugin.session.groundedness.SessionEntity._get_spans_for_agent', side_effect=mock_get_spans_for_agent):
    ...
    async def test_groundedness_agent_no_conversation_data(self):
        ...
        with patch.object(type(session), 'agent_stats', new_callable=lambda: property(lambda self: {"silent_agent": {}})), \
             patch('mce_metrics_plugin.session.groundedness.SessionEntity.get_agent_conversation_text', return_value=""):  # No conversation
            results = await metric.compute(session, agent_computation=True)
    ...
    async def test_groundedness_agent_no_model(self):
        ...
        with patch.object(type(session), 'agent_stats', new_callable=lambda: property(lambda self: {"agent_a": {}})), \
             patch('mce_metrics_plugin.session.groundedness.SessionEntity.get_agent_conversation_text', return_value="Agent conversation"), \
             patch('mce_metrics_plugin.session.groundedness.SessionEntity._get_spans_for_agent', return_value=[Mock(span_id="span_1")]):
            results = await metric.compute(session, agent_computation=True)
    ...
    async def test_groundedness_agent_exception_handling(self):
        ...
        with patch.object(type(session), 'agent_stats', new_callable=lambda: property(lambda self: {"problematic_agent": {}})), \
             patch('mce_metrics_plugin.session.groundedness.SessionEntity.get_agent_conversation_text', side_effect=Exception("Conversation extraction failed")):
            results = await metric.compute(session, agent_computation=True)
        ...
        result = results[0]
        assert result.success is False
        assert "Error computing groundedness for agent problematic_agent" in result.error_message
    ...
    def test_groundedness_agent_level_coordinator_skipped(self):
        ...
        with patch('mce_metrics_plugin.session.groundedness.get_agent_role_and_skip_decision') as mock_role_func:
    ...
    def test_groundedness_agent_level_processor_evaluated(self, mock_role_detection):
        ...
        with patch.object(type(session), 'agent_stats', new_callable=lambda: property(lambda self: {"coder": Mock()})), \
             ...
             patch('mce_metrics_plugin.session.groundedness.SessionEntity.get_agent_conversation_text', return_value="User: Calculate 2+2\nAssistant: Based on mathematical rules, the result is 4."):
    ...
    def test_groundedness_agent_level_filtering_disabled(self, mock_role_detection):
        ...
        with patch.object(type(session), 'agent_stats', new_callable=lambda: property(lambda self: {"supervisor": Mock()})), \
             patch.object(session, '_get_spans_for_agent', return_value=[create_agent_span()]), \
             patch('mce_metrics_plugin.session.groundedness.SessionEntity.get_agent_conversation_text', return_value="System: Route tasks\nAssistant: {\"next\": \"finish\"}"):
