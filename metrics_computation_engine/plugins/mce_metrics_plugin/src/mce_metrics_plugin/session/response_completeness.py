        ...
        for agent_name in session.agent_stats.keys():
            try:
                ...
                if should_skip:
                    ...
                    logger.info(f"Skipping agent '{agent_name}' for Response Completeness metric: {role_metadata.get('skip_reason', 'Detected as coordinator agent')}")
                    continue
                ...
                prompt = RESPONSE_COMPLETENESS_PROMPT.format(conversation=agent_conversation)
                ...
                logger.error(f"ERROR in response completeness computation for agent {agent_name}:")
