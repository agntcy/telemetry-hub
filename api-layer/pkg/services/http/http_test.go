...
// Add missing GetTraceBySpanNameSessionIDAndAgent method to satisfy services.DataService interface
func (m *MockDataService) GetTraceBySpanNameSessionIDAndAgent(spanName, sessionID, agentID string) (models.OtelTraces, error) {
	args := m.Called(spanName, sessionID, agentID)
...
// Add missing GetExecutionGraphBySessionID method to satisfy services.DataService interface
...
func TestSessions(t *testing.T) {
	t.Run("GET /traces/sessions with valid time range should return sessions", func(t *testing.T) {
		...
		// Mock the new paginated method with default parameters (page=0, limit=50, nameFilter=nil)
	...
	t.Run("GET /traces/sessions with start_time after end_time should return bad request", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)

		// start_time is after end_time (invalid)
		req := httptest.NewRequest(http.MethodGet, "/traces/sessions?start_time=2023-06-25T18:04:05Z&end_time=2023-06-25T15:04:05Z", nil)
		w := httptest.NewRecorder()
		...
		assert.Contains(t, errorResponse.Reason, "start_time must be before end_time")
	})

	t.Run("GET /traces/sessions without start_time should return bad request", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)

		req := httptest.NewRequest(http.MethodGet, "/traces/sessions?end_time=2023-06-25T18:04:05Z", nil)
		w := httptest.NewRecorder()
		...
		assert.True(t, errorResponse.Error)
		assert.Equal(t, http.StatusBadRequest, errorResponse.Status)
		assert.Contains(t, errorResponse.Reason, "start_time parameter is required")
	})

	t.Run("GET /traces/sessions without end_time should return bad request", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)

		req := httptest.NewRequest(http.MethodGet, "/traces/sessions?start_time=2023-06-25T15:04:05Z", nil)
		w := httptest.NewRecorder()
		...
		assert.True(t, errorResponse.Error)
		assert.Equal(t, http.StatusBadRequest, errorResponse.Status)
		assert.Contains(t, errorResponse.Reason, "end_time parameter is required")
	})

	t.Run("GET /traces/sessions without time parameters should return bad request", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)

		req := httptest.NewRequest(http.MethodGet, "/traces/sessions", nil)
		w := httptest.NewRecorder()
	...
	t.Run("GET /traces/sessions with service error should return internal server error", func(t *testing.T) {
		...
		mockDataService.On("GetSessionIDSUniqueWithPagination",
			mock.AnythingOfType("time.Time"),
			mock.AnythingOfType("time.Time"),
			0, 50, (*string)(nil)).Return([]models.SessionUniqueID{}, 0, errors.New("database error"))
		...
		// Verify that the response is now in JSON format with the new error structure
		var errorResponse ErrorResponse
		err := json.Unmarshal(w.Body.Bytes(), &errorResponse)
		assert.NoError(t, err)
		assert.True(t, errorResponse.Error)
...
func TestTraces(t *testing.T) {
	...
	t.Run("GET /traces/session/{session_id} with no traces found should return not found", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)
		router := createTestRouter(server)

		sessionID := "nonexistent_session"
		mockDataService.On("GetTracesBySessionID", sessionID).Return([]models.OtelTraces{}, models.NewNotFoundError("no traces found for session ID: "+sessionID))
		...
		// Verify that the response is in JSON format with the correct error structure
		var errorResponse ErrorResponse
		err := json.Unmarshal(w.Body.Bytes(), &errorResponse)
		assert.NoError(t, err)
		assert.True(t, errorResponse.Error)
		assert.Equal(t, http.StatusNotFound, errorResponse.Status)
		assert.Contains(t, errorResponse.Reason, "no traces found for session ID")
...
func TestSessionSpans(t *testing.T) {
	t.Run("GET /traces/session/spans with valid session_ids should return traces", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)

		sessionIDs := []string{"session_abc123", "session_def456"}
		expectedTraces := map[string][]models.OtelTraces{
			"session_abc123": {
				{TraceId: "trace_1", SpanName: "span_1"},
				{TraceId: "trace_2", SpanName: "span_2"},
			},
			"session_def456": {
				{TraceId: "trace_3", SpanName: "span_3"},
			},
		}
		var skippedSessions []string

		mockDataService.On("GetTracesBySessionIDs", sessionIDs).Return(expectedTraces, skippedSessions, nil)

		url := "/traces/session/spans?session_ids=session_abc123,session_def456"
		req := httptest.NewRequest(http.MethodGet, url, nil)
		w := httptest.NewRecorder()

		server.SessionSpans(w, req)

		assert.Equal(t, http.StatusOK, w.Code)
		assert.Equal(t, "application/json", w.Header().Get("Content-Type"))

		var response models.SessionSpansResponse
		err := json.Unmarshal(w.Body.Bytes(), &response)
		assert.NoError(t, err)
		assert.Equal(t, expectedTraces, response.Data)
		assert.Equal(t, skippedSessions, response.NotFoundSessionIds)

		mockDataService.AssertExpectations(t)
	})

	t.Run("GET /traces/session/spans with missing session_ids should return bad request", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)

		req := httptest.NewRequest(http.MethodGet, "/traces/session/spans", nil)
		w := httptest.NewRecorder()
		...
		assert.NoError(t, err)
		assert.True(t, errorResponse.Error)
		assert.Contains(t, errorResponse.Reason, "session_ids parameter is required")
	})

	t.Run("GET /traces/session/spans with too many session_ids should return bad request", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)

		// Create 51 session IDs to exceed the limit
		var sessionIDs []string
		for i := 0; i < 51; i++ {
			sessionIDs = append(sessionIDs, fmt.Sprintf("session_%d", i))
		}
		sessionIDsParam := strings.Join(sessionIDs, ",")

		url := "/traces/session/spans?session_ids=" + sessionIDsParam
		req := httptest.NewRequest(http.MethodGet, url, nil)
		w := httptest.NewRecorder()
		...
		assert.NoError(t, err)
		assert.True(t, errorResponse.Error)
		assert.Contains(t, errorResponse.Reason, "Too many session IDs provided (maximum 50)")
	})

	t.Run("GET /traces/session/spans with some skipped sessions should return partial results", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)

		sessionIDs := []string{"session_abc123", "session_def456", "nonexistent_session"}
		expectedTraces := map[string][]models.OtelTraces{
			"session_abc123": {
				{TraceId: "trace_1", SpanName: "span_1"},
			},
		}
		skippedSessions := []string{"session_def456", "nonexistent_session"}

		mockDataService.On("GetTracesBySessionIDs", sessionIDs).Return(expectedTraces, skippedSessions, nil)

		url := "/traces/session/spans?session_ids=session_abc123,session_def456,nonexistent_session"
		req := httptest.NewRequest(http.MethodGet, url, nil)
		w := httptest.NewRecorder()
	...
	t.Run("GET /traces/session/spans with service error should return internal server error", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)

		sessionIDs := []string{"session_abc123"}
		mockDataService.On("GetTracesBySessionIDs", sessionIDs).Return(map[string][]models.OtelTraces{}, []string{}, errors.New("database error"))

		url := "/traces/session/spans?session_ids=session_abc123"
		req := httptest.NewRequest(http.MethodGet, url, nil)
		w := httptest.NewRecorder()
	...
	t.Run("POST /traces/session/spans should return method not allowed", func(t *testing.T) {
		mockDataService := new(MockDataService)
		server := createTestServer(mockDataService)

		req := httptest.NewRequest(http.MethodPost, "/traces/session/spans?session_ids=session_abc123", nil)
		w := httptest.NewRecorder()
		...
		assert.NoError(t, err)
		assert.True(t, errorResponse.Error)
		assert.Contains(t, errorResponse.Reason, "Method not allowed")
	})
}
